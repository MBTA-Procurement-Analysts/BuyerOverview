...
function (input, output) {
  ...
  reactive_raw_buyer_dt_req <- reactive({
    filter(raw_buyer_req, Buyer == input$buyer) %>%
      select(`Req ID`, Status, Requester, `Req Date`, Duration, `Approval_Date`, `PO No.`, `Amount`)
  })
  ...
  output$pg1_main_plot_by_buyer <- renderPlotly(plot_ly(data = reactive_raw_buyer_dt_req(),
                                                        x = ~Duration,
                                                        y = ~Amount,
                                                        type = 'scatter',
                                                        mode = 'markers') %>%
  ...
  output$pg1_viewing_buyer_sum <- renderText({paste("Buyer Dollar Amount YTD:",
                                                    currency(
                                                      summarise(
                                                        reactive_raw_buyer_req(), Sum = sum(Amount))[[1]]),
                                                    sep = '\n')})
